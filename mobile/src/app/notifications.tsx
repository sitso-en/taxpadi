...
export default function NotificationsScreen() {
  ...
  const getIconConfig = (title: string) => {
    ...
    return { name: "notifications-outline", color: "#6B7280", bg: "#EDE8E3" };
  };
  ...
  const handleClearAll = async () => {
    ...
    try {
      ...
      showToast(error?.response?.data?.message ?? "Unable to clear notifications.", "error");
    } finally {
  ...
  const handleMarkAllAsRead = async () => {
    ...
    try {
      ...
      showToast(error?.response?.data?.message ?? "Unable to mark notifications as read.", "error");
    } finally {
      setMarkingAll(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <Text style={styles.subtitle}>
        Stay updated with reminders and tax activity.
      </Text>
      ...
      {notifications.length > 0 && (
        <Text style={styles.hintText}>Long press a notification to delete it.</Text>
      )}
    </ScrollView>
  );
}
